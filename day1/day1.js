...
const digArray = ["one", "1", "two", "2", "three", "3", "four", "4", "five", "5", "six", "6", "seven", "7", "eight", "8", "nine", "9"];

let inputArray = ["7gksfives",
"41nine",
"rflzninemfs1",
"foureight6",
"hhnineight"]
...
inputArray.forEach(inputString => {
    ...
    digArray.forEach(element => {
        let currentIndex = inputString.indexOf(element)
        let elementArr = []
        if (currentIndex !== -1){
            elementArr.push(currentIndex);
        }
        indexesArr.push(elementArr);
        console.log(indexesArr)
    })

    // if (digArray.some(v => inputString.includes(v))){
    //     currentVal.push(inputString[i])
    //     console.log("index" + [i])
    ...
    // }
    // let lastitemString = currentVal.pop();
    // let currentNum = currentVal[0] + lastitemString
    //currentNum = parseInt(currentNum)
    // numsArr.push(currentNum)
    //numsArr.push(currentVal)
    return indexesArr
});
